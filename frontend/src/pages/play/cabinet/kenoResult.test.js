...
test("sub-stake payout is presented as a return, not a win", () => {
  expect(kenoPayoutLabel(36, 100, money)).toBe("RETURN 36.00 CHIPS");
});

test("only a payout above stake is presented as a win", () => {
  expect(kenoPayoutLabel(100, 100, money)).toBe("STAKE RETURN 100.00 CHIPS");
  expect(kenoPayoutLabel(251, 100, money)).toBe("WIN 251.00 CHIPS");
});
