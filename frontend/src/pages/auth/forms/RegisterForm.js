...
export default function RegisterForm({ onSwitchPanel, showTitle = false }) {
  ...
  const goPanel = (panel, registrationSubmitted = false) => {
    if (onSwitchPanel) onSwitchPanel(panel, { registrationSubmitted });
    else navigate(frontPathForAuthPanel(panel, registrationSubmitted));
  };
  ...
  const validate = () => {
    ...
    else if (normalizedEmail.length > 254 || emailInput?.validity?.typeMismatch) errors.email = "Enter a valid email address";
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      const generatedUsername = loginIdFromPhone(normalized);
  ...
  return (
    <div data-testid="frontpage-register-panel">
      {showTitle && (
        <div className="mb-5">
      ...
      <form ref={formRef} onSubmit={submit} noValidate className="space-y-4">
        ...
        <Field label="Email address" htmlFor="reg-email" error={validationErrors.email}>
      ...
      <p className="mt-5 text-center text-sm text-white/60">Already registered?{" "}
        <button type="button" onClick={() => goPanel(AUTH_PANELS.LOGIN)} className="text-primary font-semibold hover:underline">Log in</button>
      </p>
    </div>
  );
}
