...
export default function BankDetailsPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      toast.success("Payout account saved securely");
  ...
  return (
    <PageTransition className="space-y-5">
      ...
      <form onSubmit={submit} autoComplete="off" className="space-y-4 rounded-2xl border border-white/10 bg-card/55 p-4" data-testid="bank-details-form">
        ...
        <Button type="submit" disabled={busy || !available} className="h-12 w-full rounded-xl font-bold">{busy ? "Saving securely…" : !available ? "Withdrawals temporarily unavailable" : "Add payout account"}</Button>
