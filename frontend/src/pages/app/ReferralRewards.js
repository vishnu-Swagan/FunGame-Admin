...
function taskReward(task) {
  if (String(task.reward_type || "").toUpperCase() === "REAL_CHIPS" && Number(task.reward_chips) > 0) {
    return `${formatChips(task.reward_chips)} real chips`;
  }
    ...
    <PageTransition className="-mx-4 -my-4 min-h-[100dvh] overflow-x-clip bg-background px-4 pb-8 pt-4" data-testid="referral-rewards-page">
      <div className="mx-auto max-w-xl">
        ...
        <motion.section initial={reducedMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={reducedMotion ? { duration: 0 } : { duration: 0.26 }} className="mt-5 overflow-hidden rounded-3xl border border-primary/30 bg-card/70 p-5 shadow-[0_24px_64px_rgba(0,0,0,.4)]">
          <div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-primary">{referral.automatic_rewards ? "Real chips earned" : "Verified reward balance"}</p><h1 className="mt-2 text-3xl font-extrabold tabular-nums">{formatChips(referral.verified_reward_chips)}</h1></div><span className="flex h-14 w-14 items-center justify-center rounded-2xl border border-primary/30 bg-primary/10"><Gift className="h-7 w-7 text-primary" /></span></div>
          {referral.automatic_rewards ? <div className="mt-5 grid grid-cols-2 gap-3" data-testid="automatic-referral-rates"><div className="rounded-xl border border-white/10 bg-black/15 p-3"><p className="text-[10px] uppercase tracking-wide text-white/40">Direct referral</p><p className="mt-1 text-xl font-extrabold text-primary">{referral.direct_percent || 10}%</p><p className="mt-1 text-[10px] text-white/45">First deposit · max {formatChips(referral.direct_cap_chips || 500)}</p></div><div className="rounded-xl border border-white/10 bg-black/15 p-3"><p className="text-[10px] uppercase tracking-wide text-white/40">Second level</p><p className="mt-1 text-xl font-extrabold text-primary">{referral.second_level_percent || 5}%</p><p className="mt-1 text-[10px] text-white/45">First deposit through your referral</p></div><p className="col-span-2 text-xs leading-relaxed text-white/55">Rewards are verified and added automatically as withdrawable real chips. No claim step is required.</p></div> : <><div className="mt-5 flex items-end justify-between gap-3"><p className="text-sm text-white/55">Claim threshold <strong className="tabular-nums text-white">{formatChips(referral.claim_threshold_chips)}</strong></p><strong className="tabular-nums text-3xl text-primary">{percent}%</strong></div><div role="progressbar" aria-label="Verified referral reward progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={percent} aria-valuetext={`${percent} percent verified`} className="mt-3 h-3 overflow-hidden rounded-full border border-white/10 bg-black/35"><motion.div className="h-full w-full origin-left rounded-full bg-primary" initial={false} animate={{ scaleX: percent / 100 }} transition={reducedMotion ? { duration: 0 } : { duration: 0.4 }} /></div><div className="mt-3 flex justify-between text-xs text-white/45"><span className="tabular-nums">{formatChips(referral.remaining_chips)} remaining</span><span className="tabular-nums">{formatChips(referral.pending_reward_chips)} pending verification</span></div><Button type="button" onClick={claim} disabled={!referral.claimable || busy === "claim"} className={`mt-5 h-12 w-full rounded-xl text-base font-extrabold ${referral.claimable ? "bg-[hsl(var(--emerald))] text-slate-950 hover:bg-[hsl(var(--emerald))]/90" : ""}`} data-testid="claim-referral-button">{busy === "claim" ? "Confirming claim…" : referral.claimable ? "Claim verified reward" : referralClaimDisabledCopy(referral.claim_disabled_reason)}</Button></>}
        </motion.section>
        ...
        <section className="mt-4 rounded-2xl border border-white/10 bg-card/50 p-4"><div className="flex items-center justify-between"><div><h2 className="font-bold">{referral.automatic_rewards ? "Automatic credits" : "Reward tasks"}</h2><p className="mt-1 text-xs text-white/45">{referral.automatic_rewards ? "First-deposit referral history" : "Server-verified fixed rewards"}</p></div><span className="rounded-full border border-white/12 bg-white/5 px-2 py-1 text-[10px] text-white/50">{tasks.length} {referral.automatic_rewards ? "credits" : "tasks"}</span></div><div className="mt-4 space-y-3">{tasks.length ? tasks.map((task, index) => { const eligible = serverAllowsAppeal(task) && !renderedAppealRelationships.has(task.referral_id); if (eligible) renderedAppealRelationships.add(task.referral_id); return <ReferralTask key={task.id || `${task.task_type}:${index}`} task={task} appealEligible={eligible} appealReason={appealReasons[task.referral_id] || ""} appealError={appealErrors[task.referral_id] || ""} appealBusy={busy === `appeal:${task.referral_id}`} onAppealReason={(referralId, value) => setAppealReasons((current) => ({ ...current, [referralId]: value }))} onAppeal={appeal} />; }) : <p className="rounded-xl border border-white/8 bg-black/10 p-4 text-center text-sm text-white/45">{referral.automatic_rewards ? "No referral deposits have earned a reward yet." : "No referral tasks have been published."}</p>}</div></section>

        <section className="mt-4 rounded-2xl border border-amber-300/20 bg-amber-300/[.06] p-4 text-xs leading-relaxed text-amber-50/75"><p className="flex items-center gap-2 font-bold text-amber-100"><ShieldAlert className="h-4 w-4" />Fair-use and privacy notice</p><p className="mt-2">One inviter is allowed per eligible new player. {referral.automatic_rewards ? "Only the referred player’s first verified deposit earns a reward; self-referrals and duplicate reward events are rejected." : "Registration and deposit tasks pass identity, payment and anti-fraud verification before rewards become claimable. Device data is only one review signal and is not the sole decision."}</p><p className="mt-2"><Link to="/support" className="font-bold underline">Ask for support</Link> · <Link to="/responsible-play" className="font-bold underline">Responsible play</Link></p></section>
      </div>
    </PageTransition>
  );
}
