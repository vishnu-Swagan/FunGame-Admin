...
function VideoHero({ navigate, userName }) {
  ...
  return (
    <div className="-mx-4 -mt-4" data-testid="frontpage-lobby-hero" onPointerDown={firstTouch}>
        ...
        <div className="relative px-5 pt-3 pb-4" style={{ background: "linear-gradient(180deg, #05070f, #0a0e1a)" }}>
          ...
          <div className="mt-3 flex items-center gap-2.5">
            <button
              ...
              onClick={() => { sfx.chip && sfx.chip(); document.getElementById("frontpage-game-lobby")?.scrollIntoView({ behavior: "smooth", block: "start" }); }}
            ...
            <button
              ...
              onClick={() => document.getElementById("frontpage-game-lobby")?.scrollIntoView({ behavior: "smooth", block: "start" })}
...
export default function PlayerLobby() {
  ...
  const { user } = useAuth();
  useEffect(() => {
    if (searchParams.get("tab") === "games") {
      const t = window.setTimeout(() => {
        document.getElementById("frontpage-game-lobby")?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 50);
  ...
  useEffect(() => {
    let cancelled = false;
    promoApi.state().then((data) => {
      if (cancelled) return;
      setPromo(data);
      const shown = sessionStorage.getItem("chakri-overlay-shown");
  ...
  return (
    <PageTransition className="space-y-6" data-testid="frontpage-active-lobby">
      ...
      {loading ? (
        ...
        <>
          ...
          <section>
            <SectionTitle
              action={
                <button data-testid="home-see-all-games" onClick={() => document.getElementById("frontpage-game-lobby")?.scrollIntoView({ behavior: "smooth", block: "start" })} className="text-xs font-semibold text-primary hover:underline flex items-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded">
          ...
          {/* Full lobby grid (merged from Games.js) */}
          <section id="frontpage-game-lobby" data-testid="frontpage-game-lobby" className="space-y-4 scroll-mt-24">
            <div>
              ...
              {["All", ...CATEGORY_ORDER].map((c) => (
                <button
                  ...
                  className={`shrink-0 rounded-full px-4 py-2 min-h-[36px] text-xs font-semibold border transition-[background-color,color] duration-150 ${
                    lobbyCat === c ? "bg-primary text-primary-foreground border-primary" : "bg-white/5 text-white/70 border-white/10 hover:bg-white/10"
                  }`}
                >
                  {c}
                </button>
            ...
            <div data-testid="games-grid" className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {(lobbyCat === "All" ? games : games.filter((g) => g.category === lobbyCat)).map((g) => (
                <GameCard key={`lobby-${g.slug}`} game={g} isFavorite={favorites.includes(g.slug)} onToggleFavorite={toggleFavorite} />
              ))}
            </div>
          </section>
