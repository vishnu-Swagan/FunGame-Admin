  ...
  return <PageTransition className="space-y-4" data-testid="payment-hub">
    ...
    <Tabs defaultValue="gateways" className="space-y-4">
      ...
      <TabsContent value="gateways" className="space-y-4">
        ...
        <Panel title="Add provider configuration" subtitle="Super Admins can add providers, save credentials, and copy webhook URLs after save. New gateways stay disabled until Enable (needs PAYMENTS_V2_ENABLED). Live wallet posting still follows real-money flags.">
          {canCreateGateway ? <form data-testid="add-provider-form" onSubmit={createGateway} className="space-y-4">
            ...
            <p className="rounded-lg border border-amber-300/20 bg-amber-300/8 p-3 text-xs text-amber-100">After save, copy the provider webhook URL into the provider dashboard. Enable is available only when PAYMENTS_V2_ENABLED is on. Provider funds cannot post to player wallets while REAL_MONEY, DEPOSITS, or WITHDRAWALS are false.</p>
            <Button disabled={Boolean(busy)}><Plus className="mr-2 h-4 w-4" />{busy === "create-gateway" ? "Saving…" : "Save provider"}</Button>
      ...
      <TabsContent value="routing" className="space-y-4">
        <Panel title="Routing configuration" subtitle="Routing is stored configuration until real-money flags are certified. Player traffic does not go live without those flags.">
          {routes.length ? <div className="space-y-2">{routes.map((item) => <div key={item.id} className="grid items-center gap-2 rounded-xl border border-white/8 bg-black/10 p-3 text-sm sm:grid-cols-[1fr_.7fr_.7fr_auto]"><strong>{item.name}</strong><span>{item.direction} · {item.payment_method}</span><span>{item.currency} · priority {item.priority}</span><Status tone={item.is_enabled ? "healthy" : "neutral"}>{item.is_enabled ? "Enabled" : "Disabled"}</Status></div>)}</div> : <p className="text-sm text-white/45">No routing configuration records.</p>}
        </Panel>
        <Panel title="Approvals" subtitle="Shows stored approval status. Enable/approve still needs PAYMENTS_V2_ENABLED; player traffic still follows certified real-money flags.">
          {approvals.length ? <div className="space-y-2">{approvals.map((item) => {
            const approvalStatus = String(item.status || "UNKNOWN").toUpperCase();
            const tone = approvalStatus === "PENDING" ? "warning" : ["APPROVED", "EXECUTED", "COMPLETED"].includes(approvalStatus) ? "healthy" : "neutral";
            return <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/8 bg-black/10 p-3 text-sm"><div><strong>{item.action_type}</strong><p className="font-mono text-[10px] text-white/40">{item.target_type} · {item.target_id}</p></div><Status tone={tone}>{approvalStatus}</Status></div>;
          })}</div> : <p className="text-sm text-white/45">No stored approval requests.</p>}
        </Panel>
        <Panel title="Create routing configuration" subtitle="Amounts are in paise; ₹1,000 is entered as 100000. Routing is stored config until real-money flags are certified.">
          {canManageRoutes ? <form onSubmit={createRoute} className="grid gap-3 md:grid-cols-3"><Input required placeholder="Route name" value={routeForm.name} onChange={(event) => setRouteForm({ ...routeForm, name: event.target.value })} /><Select value={routeForm.gateway_id} onValueChange={(value) => setRouteForm({ ...routeForm, gateway_id: value })}><SelectTrigger><SelectValue placeholder="Provider" /></SelectTrigger><SelectContent>{gateways.map((gateway) => <SelectItem key={gateway.id} value={gateway.id}>{gateway.display_name}</SelectItem>)}</SelectContent></Select><Select value={routeForm.direction} onValueChange={(value) => setRouteForm({ ...routeForm, direction: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="PAYIN">Pay-in</SelectItem><SelectItem value="PAYOUT">Payout</SelectItem></SelectContent></Select><Input aria-label="Minimum amount in paise" type="number" min="1" value={routeForm.min_amount_minor} onChange={(event) => setRouteForm({ ...routeForm, min_amount_minor: event.target.value })} /><Input aria-label="Maximum amount in paise" type="number" min="1" value={routeForm.max_amount_minor} onChange={(event) => setRouteForm({ ...routeForm, max_amount_minor: event.target.value })} /><Button disabled={Boolean(busy) || !routeForm.gateway_id}><Route className="mr-2 h-4 w-4" />Save route</Button></form> : <p className="text-sm text-white/45">Read-only access. Route changes require a Super Admin with route-management permission.</p>}
        </Panel>
      </TabsContent>

      <TabsContent value="webhooks"><Panel title="Webhook URLs for provider dashboards" subtitle="Copy these URLs into each provider dashboard. Received events are listed below; live wallet posting still follows real-money flags.">
          <div className="space-y-3" data-testid="webhook-registration-urls">
            {gateways.length ? gateways.map((gateway) => {
              const v2Url = webhookUrlFor(gateway, status);
              const v1Url = v1WebhookUrlFor(gateway, status);
              return <div key={gateway.id} className="space-y-2 rounded-xl border border-white/8 bg-black/10 p-3" data-testid={`webhook-urls-${gateway.code}`}>
                <p className="text-sm font-bold">{gateway.display_name} <span className="font-mono text-[10px] text-white/40">{gateway.code}</span></p>
                <CopyableUrl label="V2 webhook URL" value={v2Url} testId={`webhook-tab-v2-${gateway.code}`} />
                <CopyableUrl label="V1 webhook URL" value={v1Url} testId={`webhook-tab-v1-${gateway.code}`} />
                {!v2Url && !v1Url && <p className="text-xs text-amber-100">Set PAYMENT_WEBHOOK_PUBLIC_BASE_URL to a public https origin in Render to display webhook URLs.</p>}
              </div>;
            }) : <div className="space-y-2" data-testid="webhook-url-patterns">
              {webhookBase ? <>
                <CopyableUrl label="V2 webhook URL pattern" value={`${webhookBase}/api/webhooks/payments/{GATEWAY_CODE}`} testId="webhook-v2-pattern" />
                <CopyableUrl label="V1 webhook URL pattern" value={`${webhookBase}/api/payments/webhooks/{provider_name}`} testId="webhook-v1-pattern" />
              </> : <>
                <p className="text-xs text-white/55">V2 pattern: {`{base}/api/webhooks/payments/{GATEWAY_CODE}`}</p>
                <p className="text-xs text-white/55">V1 pattern: {`{base}/api/payments/webhooks/{provider_name}`}</p>
                <p className="text-xs text-amber-100">Set PAYMENT_WEBHOOK_PUBLIC_BASE_URL to a public https origin in Render to display copyable webhook URLs.</p>
              </>}
            </div>}
          </div>
          <div className="mt-4">
            <h3 className="mb-2 text-sm font-bold">Received webhook events</h3>
            {events.length ? <div className="space-y-2">{events.map((item) => <div key={item.id} className="grid gap-2 rounded-xl border border-white/8 bg-black/10 p-3 text-sm sm:grid-cols-[1fr_1fr_auto]"><div><strong>{item.provider_event_type}</strong><p className="font-mono text-[10px] text-white/40">{item.provider_event_id}</p></div><span>{when(item.received_at)}</span><Status tone={item.processing_status === "DEAD_LETTER" ? "danger" : "neutral"}>{item.processing_status}</Status></div>)}</div> : <div className="py-8 text-center text-sm text-white/45"><Webhook className="mx-auto mb-2 h-6 w-6" />No webhook events have been received.</div>}
          </div>
        </Panel></TabsContent>
