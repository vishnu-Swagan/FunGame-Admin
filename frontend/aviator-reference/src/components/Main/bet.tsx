...
import Context, { callCancelBet, callCashOut } from "../../context";
...
const Bet = ({ index, add, setAdd }: BetProps) => {
	...
	const cancellable = index === 'f' ? fcancellable : scancellable
	...
	const cashOut = index === 'f' ? state.fautoCashoutTarget : state.sautoCashoutTarget
		...
		if (type === "betAmount") {
			if (value.userInfo[index][type] + 1 > state.userInfo.balance) {
				...
				if (value.userInfo[index][type] + 1 > maxBet) {
					...
					value.userInfo[index][type] = Number(betAmount) + 1
				}
			}
		} else {
	...
	const onChangeBlur = (e: number, type: 'cashOutAt' | 'decrease' | 'increase' | 'singleAmount') => {
		let value = state;
		if (type === "cashOutAt") {
			let normalized = Math.round(e * 100) / 100;
			if (e < 1.01) {
				normalized = 1.01;
			} else if (e > 1000) {
				normalized = 1000;
			...
			update({
				[`${index}autoCashoutTarget`]: normalized,
				userInfo: {
					...state.userInfo,
					[index]: { ...state.userInfo[index], target: normalized },
				},
			});
			return;
		} else {
	...
	const onBetClick = (s: boolean) => {
		if (s && gameType === "auto" && autoCashoutState) {
			update({
				[`${index}autoCashoutTarget`]: cashOut,
				userInfo: {
					...state.userInfo,
					[index]: { ...state.userInfo[index], target: cashOut },
				},
			});
		}
	...
	const onAutoBetClick = (_betState: boolean) => {
		update({
			[`${index}autoCashoutTarget`]: cashOut,
			[`${index}autoStartBalance`]: _betState ? state.userInfo.balance : 0,
			userInfo: {
				...state.userInfo,
				[index]: {
					...state.userInfo[index],
					auto: _betState,
					target: autoCashoutState ? cashOut : state.userInfo[index].target,
				},
			},
		});
			...
			<div className="controls">
				...
				<div className="first-row">
								...
								<div className="input">
									{betState || betted ?
										...
										<input type="number" value={Number(myBetAmount)}
											onChange={e => {
												const normalized = Math.min(maxBet, Math.max(0, Math.round(Number(e.target.value) || 0)));
												update({ ...state, userInfo: { ...state.userInfo, [`${index}`]: { ...state.userInfo[index], betAmount: normalized } } });
											}}
											onBlur={() => {
												if (betAmount < minBet) update({ ...state, userInfo: { ...state.userInfo, [`${index}`]: { ...state.userInfo[index], betAmount: minBet } } });
											}}></input>
									}
								</div>
								<div className="buttons">
					...
					<div className="buttons-block">
						{betted ? cancellable ?
							<button className="btn-danger" onClick={() => callCancelBet(index)}><label>CANCEL</label></button>
							: GameState === "PLAYING" ?
							...
							<div className="cashout-block">
								<div className="cashout-switcher">
									...
									{betted || betState ? (
										...
										<div onClick={() => {
											const enabled = !autoCashoutState;
											update({
												[`${index}autoCashoutState`]: enabled,
												[`${index}autoCashoutTarget`]: cashOut,
												userInfo: {
													...state.userInfo,
													[index]: {
												...
												{autoCashoutState && !betState ? (
													<input type="number"
														onChange={(e) => {
															...
															update({
																[`${index}autoCashoutTarget`]: target,
																userInfo: {
																	...state.userInfo,
																	[index]: { ...state.userInfo[index], target },
																},
															});
														}}
														value={cashOut}
