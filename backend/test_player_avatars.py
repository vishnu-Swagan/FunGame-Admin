    ...
    async def test_self_service_account_creation_persists_deterministic_preset(self):
        ...
        with (
            patch.dict(os.environ, {
                "APP_ENV": "test",
                "OTP_PEPPER": "avatar-test-only-otp-pepper-with-at-least-32-characters",
            }),
        ...
        created = await self.database.users.find_one({"pending_phone": phone})
        # Admin-reviewed registration deliberately keeps submitted contacts
        # provisional and the account inactive until an operator approves it.
        self.assertEqual(created["status"], "PENDING")
        self.assertEqual(created["activation_mode"], routes_auth.ADMIN_REVIEW_ACTIVATION_MODE)
        self.assertFalse(created["contact_verified"])
        self.assertEqual(created["pending_phone"], phone)
        self.assertEqual(created["pending_email"], "avatar.player@example.com")
