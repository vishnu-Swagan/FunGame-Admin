...
from deps import require_admin, require_player
import free_cash
import wager
...
async def promo_state(user=Depends(require_player)):
    return {
...
async def claim_free_cash(user=Depends(require_player)):
...
async def admin_patch_promo(request: Request, admin=Depends(require_admin)):
    ...
    # Free-cash knobs live on the same promo-settings doc.
