...
async def rummy_join(body: JoinRequest, user: dict = Depends(require_active_player)):
    ...
    async def join_transaction(session):
        ...
        if existing_seat and existing_room and existing_room.get("state") not in ("ROUND_SETTLED", "CANCELLED"):
            # Old preview builds could leave placeholder bot rows without a
            # user_id.  They made an otherwise valid practice membership crash
            # on resume, or occupy the five-row projection limit and render all
            # seats as EMPTY.  Repair only the unambiguous one-human waiting
            # room; active rounds are filtered defensively by _public_state.
            if (
                body.mode == "PRACTICE"
                and existing_room.get("mode") == "PRACTICE"
