# Keno production launch checklist

Product: Chakri.Casino live Keno cabinet
...
- [x] 🤖 **Build the production frontend** — 3 minutes. Execute the CRACO production build with source maps disabled. **You’ll know it worked when:** compilation finishes successfully and emits the Keno CSS/JavaScript bundle.
- [x] 🤖 **Push the reviewed release** — 1 minute. Commit only the Keno runtime, tests, catalogue migration, and this checklist, then push `main`. **You’ll know it worked when:** GitHub `main` points to the release commit.
...
- [ ] 🤖 **Verify Render frontend** — 5–10 minutes. Confirm the production asset manifest references the new bundle and open `/games/keno/play` through an active player session. **You’ll know it worked when:** the live route shows LIVE MODE and the reference-matched cabinet instead of the legacy Keno screen.
