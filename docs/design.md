...
## Layout
The composition uses the recording's 16:9 landscape frame. The live dealer/table stage occupies roughly the upper three quarters and the betting deck occupies the lower quarter; within that deck the compact roadmap takes the left column, main side bet the wider center, and card-count bets the right column. The visible chip rail is exactly ₹20, ₹50, ₹100, ₹200, ₹500, and ₹1,000. Canvas coordinates remain the source of truth and are scaled with one uniform transform so visuals and hit targets always agree.
